...
namespace Jelly.Repositories
{
    public class BaseRepository<T> where T : Entity, new()
    {
        ...
        public BaseRepository(JellyContext jellyContext)
        {
        ...
        {
            _jellyContext.Set<T>().AddRange(modeList);
        }
        ...
        {
            _jellyContext.Entry<T>(model).State = EntityState.Modified;
        }
        ...
        public void Edit(T model, string[] propertys)
        {
            ...
            _jellyContext.Entry<T>(model).State = EntityState.Unchanged;
            //2 将model追加到EF容器 
            foreach (var item in propertys)
            {
                _jellyContext.Entry<T>(model).Property(item).IsModified = true;
            }
        ...
        {
            _jellyContext.Set<T>().Remove(model);
        }
        ...
        {
            return _jellyContext.Set<T>().Where(whereLambda).AsQueryable();
        }
        /// <summary>
        /// 分页查询 排序
        /// </summary> 
        ...
        {
            var queryable = _jellyContext.Set<T>().Where(whereLambda);
